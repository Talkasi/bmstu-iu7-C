...
#define N_MAX 1024
/*
Реализовать функцию-фильтр, сохраняющую в массиве целых чисел только полные квадраты. Реализовать функцию, которая ставит на место после каждого двузначного числа определённое число. Принять с клавиатуры число, принять с клавиатуры массив не более, чем из 1024 элементов, применить к нему первую и вторую функции и вывести результат на экран. К каждой функции указать описание в любом удобном формате.
*/
...
void my_insertion(int number, int arr[], size_t *arr_len, int n_max) {
	for (size_t i = 0; i < *arr_len; ++i) {
		if ((arr[i] % 1000 / 100) == 0)
			elem_insert(number, i, arr, arr_len, n_max);
	}
}
